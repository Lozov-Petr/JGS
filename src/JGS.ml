...
module HO = struct
    ...
    fresh (hd tl)
      ...
      (conde
         [
           ...
           fresh n' (n === Nat.s n') (nth tl n' rez);
         ])
  ...
  module Verifier (CT : sig
    module HO : sig
      ...
      val get_superclass :
        int ilogic -> int ilogic -> jtype_injected option_injected -> goal
    ...
    let rec ( -<- ) :
     ...
     fun ( <-< ) type_a type_b res ->
      ...
      let capture_conversion :
          int ilogic ->
          jtype_injected targ_injected Std.List.injected ->
          jtype_injected targ_injected Std.List.injected Std.Option.injected ->
          goal =
       fun id targs res ->
        if need_simpified then res === !!(Some targs)
              ...
              [
                ...
                fresh (t t2 params_val)
                  ...
                  === cc_var (CT.HO.new_var ()) i
                        !!(CC_inter (t, t2))
                        !!(Some !!Null))
                  (params params_val) (nth params_val i t2);
              ]
          in
          let subst_helper :
              capture_conversion_type_injected ->
          ...
          let targs_helper :
           ...
           fun subst cc_typ res ->
            conde
              [
                fresh (t new_t) (cc_typ === !!(CC_type t))
                  (res === !!(Type new_t))
        ...
        conde
          [
            ...
            fresh super (id_a =/= id_b)
              (CT.HO.get_superclass id_a id_b super)
              (conde
                 [
...
module FO = struct
  ...
  module Verifier (CT : sig
    module HO : sig
      val decl_by_id : int ilogic -> decl_injected -> goal
